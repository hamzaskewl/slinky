...
Private payment claim links on Canton Network. Sender and claimer never learn each other's identity — privacy enforced by the Daml contract model, not the UI.
...
Run both of thse at seperate terminals, side by side would be ideal but yeah
...
3. Go to **Send** → click **Create Link** → set amount → copy the claim URL
...
privypay/
├── daml/
│   └── PrivyPay.daml          # Daml smart contracts (4 templates)
├── src/
...
Four templates in `daml/PrivyPay.daml`:
